...
// compilation cache. boundsCheckElision is runtime-scoped, so callers must use
// the engine's captured mode for serialized cache headers and keys. The public
// wazero version remains unchanged.
func GetCompilationCacheVersion(boundsCheckElision bool) string {
	if boundsCheckElision {
